...
const getUser = (userData) => {
  return async (dispatch) => {
    try {
      ...
      console.log("respuesta del id action", error);
    }
  };
};
...
const getSomePublish = (idPeople) => {
  return async (dispatch) => {
    try {
      if (idPeople === null) {
        // Despacha una acción con un payload vacío o un valor por defecto si idPeople es null
        return dispatch({
          type: SOME_PUBLISH,
          payload: [], // Aquí puedes usar un array vacío o lo que necesites
        });
      }
